...
const prefix = 'pagbank-past-due-recovery-';

integrationDescribe('PagBank PAST_DUE recovery concurrency', () => {
  ...
  it('converges one approved recovery from PAST_DUE without changing prior usage', async () => {
    const suffix = Date.now().toString();
    const planId = `${prefix}plan-${suffix}`;
    const userId = `${prefix}user-${suffix}`;
    const subscriptionId = `${prefix}subscription-${suffix}`;
    const externalSubscriptionId = `SUBS_RECOVERY_${suffix}`;
    const providerInvoiceId = `INVO_RECOVERY_${suffix}`;
    const providerPaymentId = `PAY_RECOVERY_${suffix}`;
    ...
    const oldGracePeriodEnd = new Date('2026-09-23T00:00:00.000Z');
    ...
    await prisma.subscription.create({
      data: {
        ...
        gracePeriodEnd: oldGracePeriodEnd,
      },
    });
    ...
    const providerPayment = {
      ...
      subscriptionId: externalSubscriptionId,
      orderId: `ORDER_RECOVERY_${suffix}`,
