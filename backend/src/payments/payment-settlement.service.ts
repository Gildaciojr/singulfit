...
export class PaymentSettlementService {
    ...
    return this.runSettlementTransaction(async (transaction) => {
      const approval = await this.paymentsService.approveInTransaction(
        transaction,
        {
          ...
          cardBrand: canonicalPayment.cardBrand,
          cardLastFour: canonicalPayment.cardLastFour,
        },
      );
  ...
  private async runSettlementTransaction<T>(
    ...
    for (
      let attempt = 1;
      attempt <= PaymentSettlementService.MAX_TRANSACTION_RETRIES;
      attempt += 1
    ) {
      try {
